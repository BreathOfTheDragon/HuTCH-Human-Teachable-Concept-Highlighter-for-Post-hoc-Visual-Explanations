...
super_blackened_dir = "./super_blackened_images"
super_segmented_dir = "./super_segmented_images"

if os.path.exists(super_blackened_dir):
    shutil.rmtree(super_blackened_dir)

if os.path.exists(super_segmented_dir):
    shutil.rmtree(super_segmented_dir)
    
    
os.makedirs(super_blackened_dir, exist_ok=True)
os.makedirs(super_segmented_dir, exist_ok=True)
...
def process_combinations(name_without_extension):
    ...
    specific_super_segmented_dir = os.path.join(super_segmented_dir, name_without_extension)

    create_super_image(specific_blackened_dir, specific_super_blackened_dir, 1)
    create_super_image(specific_segmented_dir, specific_super_segmented_dir, 1)
