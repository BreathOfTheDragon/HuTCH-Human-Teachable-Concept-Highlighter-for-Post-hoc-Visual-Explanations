...
# This file creates the TestData folders. Randomly sample from each common name based on the variable test_percentage for size of the sample
...
for taxon in taxons:
    ...
    for species, total_count in species_info.items():
        ...
        if not species_images:
            print(f"Warning: No images found for {species}. Skipping this species.")
            continue 

        test_count = min(max(1, int(len(species_images) * test_percentage)), len(species_images))

        test_images = random.sample(species_images, test_count)
        
        species_test_dir = os.path.join(test_folder, taxon)
        ...
        for img in test_images:
            try:
                shutil.move(str(img), os.path.join(species_test_dir, img.name))
